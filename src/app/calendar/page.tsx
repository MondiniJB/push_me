...
import { Calendar as CalendarIcon, Flame, Moon, HeartPulse, Sparkles, CheckCircle2 } from 'lucide-react';
      ...
      <section className="glass-panel space-y-4 rounded-3xl border border-zinc-800 p-5">
        ...
        <div className="flex flex-wrap items-center gap-3 border-y border-zinc-800/80 py-2.5 text-[10px] font-bold text-zinc-400">
          <span className="flex items-center gap-1 text-emerald-400">
            <span className="h-2 w-2 rounded-full bg-emerald-500" /> Entrené
          </span>
        ...
        <div className="grid grid-cols-7 gap-2 text-center">
          ...
          {daysArray.map((dayNum) => {
            ...
            return (
              ...
              >
                <span className={`text-xs font-black ${dayNum === 24 ? 'text-emerald-400' : 'text-zinc-300'}`}>
                  {dayNum}
                </span>

                <div className="flex gap-0.5">
                  {hasWorkout && <span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />}
